...
CREATE OR REPLACE VIEW vw_enrollment_report AS
SELECT
  d.dept_code,
  sem.academic_year,
  sem.level,
  ...
  c.credit,
  COALESCE(t.name, 'TBA')                              AS teacher_name,
  COUNT(e.enrollment_id)                               AS total_registered,
...
LEFT JOIN enrollment e   ON co.offering_id = e.offering_id
GROUP BY
  d.dept_code, sem.academic_year, sem.level, sem.term,
  c.course_code, c.course_name, c.credit, t.name;
