    ...
    <>
      <div className="lg:hidden fixed bottom-0 left-0 right-0 z-50 bg-white/95 backdrop-blur-md border-t border-gray-200/50 pt-2 pb-safe-bottom px-4 flex justify-around items-center shadow-[0_-10px_40px_-10px_rgba(0,0,0,0.05)]">
        {mainTabs.map(tab => {
          ...
          return (
            <NavLink key={tab.path} to={tab.path} onClick={() => setMenuOpen(false)} className="flex flex-1 flex-col items-center justify-center gap-1 relative min-w-[60px]">
              {isActive && <motion.div layoutId="bottom-nav-indicator" className="absolute top-[-9px] w-10 h-1 bg-brand-500 rounded-b-full shadow-[0_2px_4px_rgba(brand,0.5)]" />}
        ...
        <button onClick={() => setMenuOpen(true)} className="flex flex-1 flex-col items-center justify-center gap-1 relative min-w-[60px]">
          {menuOpen && <motion.div layoutId="bottom-nav-indicator" className="absolute top-[-9px] w-10 h-1 bg-brand-500 rounded-b-full shadow-[0_2px_4px_rgba(brand,0.5)]" />}
